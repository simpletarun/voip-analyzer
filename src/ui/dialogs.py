...
class IPDetailDialog(QDialog):
    def __init__(self, parent, ip: str, intel: IPInfo, stats: dict, ip_type: str):
        ...
        sections += [
            ...
            ("", ""),
            ("City", intel.city),
            ("District", intel.district or "N/A"),
            ("Region", f"{intel.region_name} ({intel.region_code})"),
            ("Country", f"{intel.country} ({intel.country_code})"),
            ("Continent", f"{intel.continent} ({intel.continent_code})"),
            ("ZIP", intel.zip or "N/A"),
            ("Timezone", intel.timezone or "N/A"),
            ("Currency", intel.currency or "N/A"),
            ("Latitude", f"{intel.lat:.4f}"),
            ("Longitude", f"{intel.lon:.4f}"),
            ("", ""),
            ("ASN", intel.asn),
            ("AS Name", intel.asname or "N/A"),
            ("", ""),
